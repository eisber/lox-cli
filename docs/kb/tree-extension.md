...
Source: https://www.loxone.com/enen/kb/tree-extension/

---

The **Loxone Tree Extension** allows the integration of various devices like sensors, actuator or lighting products that are equipped with the Loxone Tree technology.

The Miniserver features an integrated Tree interface. The Tree Extension is equipped with two Tree interfaces.

A Tree interface including wiring and devices connected to it is called a Branch. Up to 50 Tree devices can be connected per Branch with a maximum total length of 500m/1640ft.

[**Datasheet Tree Extension**](http://updatefiles.loxone.com/KnowledgeBase/Online/Common/Documents/Datasheet_TreeExtension_100218.pdf)
...
The Tree Extension is installed on a DIN rail in a suitable enclosure.
...
The Tree devices are connected to one of the two Tree interfaces. Please follow the detailed instructions for connecting Tree devices below.
...
| --- | --- | --- | --- |
| Online Status Tree Extension | Indicates whether the device can be reached by the Miniserver. Diagnostics for Air devices Diagnostics for Tree devices Diagnostics for Extensions | Digital | 0/1 |
...
| Switch off status LEDs | If checked, the status LEDs on the following switchgear units devices are switched off in normal operation: Energy Meters Tree Fan Control Tree RGBW 24V Dimmer Tree Valve Actuator Tree In the event of a fault, the devices will continue to attract attention visualize this via their status LEDs. | - |
